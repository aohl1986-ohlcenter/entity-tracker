...
describe("parseSperrliste", () => {
  ...
  it("übernimmt auch die bewusst ausgelassenen Domains", () => {
    const liste = ladeSperrliste();
    assert.ok(liste.has("lamek.de") || liste.has("osnabrueck-implantologe.de"));
    assert.ok(liste.has("ecd-immo.de"), "ECD wurde bewusst nicht angeschrieben (Verwaltung statt Vermittlung)");
  });
