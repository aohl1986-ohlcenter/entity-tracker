...
const sweep: SweepDatei = {
  ...
  kandidaten: [
    { host: "bauer-os.de", nennungen: 4, prompts: ["p"], titel: "Bauer" },
...
describe("berechneLeads", () => {
  it("trennt Leads von bereits sichtbaren Betrieben", () => {
    const e = berechneLeads(
      sweep,
      [
        { host: "bauer-os.de", titel: "Bauer", positionen: [1] },
        { host: "kaschtan-bedachungen.de", titel: "Kaschtan", positionen: [2, 5] },
      ],
      ["Dachdecker Osnabrück"],
    );
    assert.deepEqual(e.leads.map((l) => l.host), ["kaschtan-bedachungen.de"]);
    assert.deepEqual(e.sichtbar.map((s) => s.host), ["bauer-os.de"]);
...
describe("sichtbarkeitsQuote", () => {
  ...
  it("rechnet korrekt", () => {
    const e = berechneLeads(
      sweep,
      [
        { host: "bauer-os.de", titel: "", positionen: [1] },
...
describe("zitierteNichtPortale", () => {
  it("zählt Portale nicht als Firmen mit", () => {
    assert.deepEqual(zitierteNichtPortale(sweep), ["bauer-os.de"]);
  });
});
