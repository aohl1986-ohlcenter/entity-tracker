...
| Domain | Grund |
|--------|-------|
| betrieb-014.example | Impressum weist eine **ImmobilienVERWALTUNGS GmbH** aus. Verwaltung ≠ Vermittlung — meine Prompts fragten nach Maklern. Nach eigener Regel nicht anschreiben. |
| betrieb-016.example · betrieb-020.example | Geschäftsmodell vor Kontakt ungeklärt (Verwaltung/Franchise?). |
...
| Domain | Grund |
|--------|-------|
| betrieb-088.example | Impressum verweist auf **betrieb-093.example** — und betrieb-093.example wird von der KI genannt (4×). Gleiche Praxis, zweite Domain → Befund wäre falsch. |
| betrieb-037.example | Keine E-Mail im Impressum/Kontakt auffindbar (Baukasten-Seite, nur Formular). Telefonisch erreichbar. |
| betrieb-083.example | Keine E-Mail auffindbar, nur Kontaktformular. Telefonisch erreichbar. |
| betrieb-044.example | Teil eines Praxisverbunds → über #25 mitabgedeckt, keine Doppelmail. |
