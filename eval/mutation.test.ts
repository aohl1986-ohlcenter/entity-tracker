...
const SAUBERER_SWEEP: SweepDatei = {
  ...
  kandidaten: [
    { host: "betrieb-004.example", nennungen: 4, prompts: ["p"], titel: "Bauer Bedachungen" },
    { host: "betrieb-005.example", nennungen: 2, prompts: ["p"], titel: "Volmer" },
  ],
  protokoll: [
    {
...
const SAUBERE_LEADS: LeadsDatei = {
  ...
  marktGroesse: 3,
  leads: [{ host: "betrieb-001.example", titel: "Kaschtan GmbH", positionen: [2] }],
  sichtbar: [
    { host: "betrieb-004.example", titel: "Bauer", positionen: [1], nennungen: 4 },
    { host: "betrieb-005.example", titel: "Volmer", positionen: [3], nennungen: 2 },
  ],
};
...
describe("Mutationsprobe — jede Regel schlägt genau bei ihrem Fehler an", () => {
  ...
  pruefeMutation(
    ...
    (k) => {
      k.sweep.kandidaten.push({ host: "betrieb-203.example", nennungen: 2, prompts: ["p"], titel: "" });
      k.leads.leads.push({ host: "betrieb-002.example", titel: "Strübbe", positionen: [4] });
      k.leads.marktGroesse += 1;
    },
    "R4_ALIAS_DOMAIN",
  );
  ...
  pruefeMutation(
    ...
    (k) => {
      // betrieb-004.example steht in kandidaten — die erfundene Quelle hat also gewirkt
      k.sweep.protokoll[0].quellen = ["https://betrieb-004.example/erfundene-seite"];
    },
    "R7_GROUNDING",
    "blocker",
  );
